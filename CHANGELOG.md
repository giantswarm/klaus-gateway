...
- Slack read-only tool calls are auto-approved without a human click when a rule-based risk classifier rates them safe, so investigation stays frictionless. Configure with `SLACK_HITL_AUTOAPPROVE` (`green` default, `yellow`, or `off`; red-classified calls are never auto-approved, so `red` is rejected at startup) and `SLACK_HITL_ALLOWED_HOSTS`; an unrecognized `SLACK_HITL_AUTOAPPROVE` is rejected at startup rather than silently enabling green auto-approval. Auto-approval eligibility comes from the tool name alone (its verb tokens must be read-only, e.g. `get`, `list`, `describe`); the agent-controlled call arguments and free-text hint can only escalate risk, never reduce it, so a crafted argument cannot steer a mutating tool to auto-approval. Destructive `find` invocations (`-delete`, `-exec`) are never treated as read-only, and auto-approvals are capped per turn.
...
- The auto-approval classifier's database-mutation rule now matches real SQL (`DELETE FROM`, `INSERT INTO`, `UPDATE ... SET`, `DROP`, `TRUNCATE`, `ALTER TABLE`); the previous pattern required a word between the verb and the keyword and never fired. Added red rules for `$IFS` shell obfuscation and Redis `FLUSHALL`/`FLUSHDB`, extended the git rule to `git branch -D`, and the sensitive-path check collapses repeated slashes so `/etc//shadow` no longer evades it.
- `/stop` now also cancels a turn that resumed after an auto-approved tool call; previously the resumed segment kept running detached from the turn registry (and its delta producer could leak). Auto-approved resumes are capped at 20 per turn so an agent looping on read-only prompts cannot hold a thread's serialization slot forever.
