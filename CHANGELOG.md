...
- Slack HITL prompts survive a gateway restart: when a typed reply or button click finds no in-process pending state, the paused input-required task is rebuilt from kagent's task store (`GET /api/sessions/{id}/tasks`), so the decision resolves the original tool call instead of being answered with "Already answered." or starting a fresh turn that dangles it.
