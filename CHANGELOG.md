...
- `--web-enabled` flag / `KLAUS_GATEWAY_WEB_ENABLED` env / Helm `web.enabled` value gate the web channel adapter, which previously always started. The binary default stays true (local development); the chart default is false, matching `cli.enabled`. The chart now also actually renders `--cli-enabled` from `cli.enabled`, which was previously a dead value.
