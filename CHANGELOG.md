...
- Slack read-only tool calls are auto-approved without a human click when a rule-based risk classifier rates them safe, so investigation stays frictionless. Configure with `SLACK_HITL_AUTOAPPROVE` (`green` default, `yellow`, `red`, or `off`) and `SLACK_HITL_ALLOWED_HOSTS`; an unrecognized `SLACK_HITL_AUTOAPPROVE` is rejected at startup rather than silently enabling green auto-approval. The classifier reads the tool name and call arguments only (not the agent's free-text hint), and destructive `find` invocations (`-delete`, `-exec`) are never treated as read-only.
- Helm chart values for the Slack adapter's turn-progress and access-control settings, rendered as the corresponding `KLAUS_GATEWAY_SLACK_*` environment variables: `slack.progress.mode` (`auto`, `reactions`, or `text`), `slack.progress.emojis.working` / `done` / `failed`, `slack.progress.clearReactionOnDone`, `slack.hitl.autoApprove` (`green`, `yellow`, `red`, or `off`), `slack.hitl.allowedHosts`, and `slack.dropStale`. Empty emoji values, a null `clearReactionOnDone`, and an empty `autoApprove` (the defaults) let the binary decide (`auto`; `eyes` / `white_check_mark` / `x`; clear-on-done true; auto-approve `green`).
