...
- A Slack message sent before signing in is now held and answered once the user links their Giant Swarm identity, instead of being dropped so the user has to re-type it. Held per thread (latest message per thread wins), replayed through the normal turn path when the sign-in completes, and swept after 24h. Follow-ups (ordered per-thread queue, multi-user threads, durability) tracked in klaus-gateway#132.
