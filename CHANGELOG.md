...
- A Slack message sent before signing in is now held and answered once the user links their Giant Swarm identity, instead of being dropped so the user has to re-type it. Messages are held per thread as an ordered queue (up to 5 per thread, oldest dropped past the cap) and replayed in order through the normal turn path when the sign-in completes; different threads replay concurrently. A newcomer who signs in mid-thread is replayed to the initiator's access-consent prompt, not dispatched to the agent. Parked messages are swept after 24h. Restart durability (parking is in-memory) is tracked in klaus-gateway#132.
