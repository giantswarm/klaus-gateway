...
| web | object | `{}` | Ignored since the Slack-only release; accepted because the agent-platform umbrella still forwards it; removed in the next major release. |
| cli | object | `{}` | Ignored since the Slack-only release; accepted because the agent-platform umbrella still forwards it; removed in the next major release. |
| lifecycle | object | `{}` | Ignored since the Slack-only release; accepted because the agent-platform umbrella still forwards it; removed in the next major release. |
| upstream | object | `{}` | Ignored since the Slack-only release; accepted because the agent-platform umbrella still forwards it; removed in the next major release. |
| agentgateway | object | `{}` | Ignored since the Slack-only release; accepted because the agent-platform umbrella still forwards it; removed in the next major release. |
...
| routing.defaultTTL | string | `""` | Ignored since the Slack-only release; accepted because the agent-platform umbrella still forwards it; removed in the next major release. It was the TTL of a Klaus route entry; a thread's lifetime is threadTTL. |
| routing.threadTTL | string | `""` |  |
| routing.autoCreate | bool | `false` | Ignored since the Slack-only release; accepted because the agent-platform umbrella still forwards it; removed in the next major release. It created a Klaus instance on a route miss; there is no Klaus path left. |
...
| a2a.caSecret | string | `""` |  |
| a2a.caFile | string | `""` |  |
| a2a.tokenPath | string | `""` | Ignored since the Slack-only release; accepted because the agent-platform umbrella still forwards it; removed in the next major release. It named a Bearer token file for the Klaus-instance paths, which are gone. |
| a2a.fallbackIconUrlTemplate | string | `""` |  |
| a2a.saToken | object | `{}` | Ignored since the Slack-only release; accepted because the agent-platform umbrella still forwards it; removed in the next major release. It projected a ServiceAccount token for the Klaus-instance paths, which are gone; the pod mounts nothing for it any more. |
