...
func (a *Adapter) postRun(w http.ResponseWriter, r *http.Request) {
	...
	if in.Text == "" || in.SessionID == "" {
		channels.WriteJSONError(w, http.StatusBadRequest, "text and sessionId are required")
		return
	}
...
func (a *Adapter) postMessages(w http.ResponseWriter, r *http.Request) {
	...
	if in.SessionID == "" {
		channels.WriteJSONError(w, http.StatusBadRequest, "sessionId is required")
		return
	}
...
func (a *Adapter) resolveError(w http.ResponseWriter, err error) {
	if errors.Is(err, routing.ErrRouteNotFound) {
		channels.WriteJSONError(w, http.StatusNotFound, "no instance bound to this session and auto-create is disabled")
		return
	}
