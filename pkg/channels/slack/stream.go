...
//
// Only the main reply message (ts) is managed; it accumulates DeltaText
// content. When the stream ends on a DeltaPrompt, run() captures it in
// promptDelta (flushing the main buffer first) and returns nil. The caller is
// responsible for posting the approval prompt and registering the pending task.
