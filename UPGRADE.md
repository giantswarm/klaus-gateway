...
channels, `im:history` (already granted) DMs, and the two new scopes in
`deploy/slack/manifest.yaml` — `groups:history` and `mpim:history` — private channels and group
DMs. Slack applies added scopes to an existing install only on re-install, so **re-install the
...
Public channels and DMs need no re-install.
